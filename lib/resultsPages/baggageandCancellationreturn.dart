...
import 'package:tripster/resultsPages/baggageandCancellationreturn2.dart';
import 'package:tripster/resultsPages/seatMealLuggage.dart';

class BaggagePagereturn extends StatefulWidget {
  final int onwardkey;
  final int returnkey;
  const BaggagePagereturn(
      {super.key, required this.onwardkey, required this.returnkey});

  @override
  State<BaggagePagereturn> createState() => _BaggagePagereturnState();
}

class _BaggagePagereturnState extends State<BaggagePagereturn> {
  ...
  void initState() {
    ...
    super.initState();
    print(returnOnwardController.Id[widget.onwardkey].toString());
  ...
  void startErrorTimer() {
    _errorTimer = Timer.periodic(Duration(seconds: 5), (timer) {
      if (mounted) {
        setState(() {
          if (returnOnwardController.ThirdCancellationFees.length == 0) {
          } else {
            firstCancelFare = returnOnwardController.ThirdCancellationFees[0];
          }
        });
      } else {
        _stopErrorTimer();
  ...
  String calcBefore(String todate) {
    DateTime todate =
        DateTime.parse(returnOnwardController.ArrDate[widget.onwardkey])
          ...
          child: Row(
            ...
            children: [
              ClipRRect(
                ...
                child: Image(
                  image: (returnOnwardController.AirlineCode[widget.onwardkey]
                              .toString()
                              .toLowerCase() ==
                          '6e')
                      ? const AssetImage('assets/images/indigo.png')
                      : (returnOnwardController.AirlineCode[widget.onwardkey]
                                  .toString()
                                  .toLowerCase() ==
                              'uk')
                          ? const AssetImage('assets/images/vistaralogo.png')
                          : (returnOnwardController
                                      .AirlineCode[widget.onwardkey]
                                      .toString()
                                      .toLowerCase() ==
                                  'sg')
                  ...
                  children: [
                    ...
                    Text(
                      formatDate(airportController.OnewayDate.toString()
                              .split('T')[0]) +
                          " | " +
                          returnOnwardController.DepTime[widget.onwardkey]
                              .toString()
                              .split('T')[1] +
                          " - " +
                          returnOnwardController.ArrTime[widget.onwardkey]
                              .toString()
                              .split('T')[1] +
                          " | " +
                          formatDuration(returnOnwardController
                              .Duration[widget.onwardkey]) +
                          " | " +
                          returnOnwardController.Stops[widget.onwardkey]
                              .toString() +
                          " Stops",
                      style: TextStyle(
                          color: Colors.grey,
